...
import { CREATOR_KEY, creatorUpload } from "@/util/creator-upload";
...
export default function CrossfiConnect({ params }: { params: Params }) {
	...
	async function makeVideoRequest() {
		...
		setError(null);
		try {
			const res = await requestVideo(signer, Post, message, donation);

			console.log("makeVideoRequest", res, Post, message);
	...
	async function onUpload(fileName: string, data: any) {
		...
		try {
			const res = await creatorUpload(fileName, data);
			console.log("creatorUpload", res);
	...
	const PostNotClaimed = !hasCreator && !loading;

	if (PostNotClaimed) {
		return (
			<div className="flex flex-col items-center justify-center mt-8">
				<div>This Post has not been claimed by a creator yet!</div>
				<div>
					Click&nbsp;
