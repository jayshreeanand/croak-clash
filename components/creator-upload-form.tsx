...
import { getMetadataForPost, registerPost } from "@/lib/contract/interact";
...
const formSchema = z.object({
	post: z.string().min(3, {
		message: "Creator page post must be at least 3 characters",
	}),
...
function CreatorForm() {
	...
	const setDemoData = async () => {
		form.setValue("title", "How to build a deFi app");
		form.setValue("post", "defi-tutorial");
		form.setValue("description", getPlaceholderDescription());
		form.setValue(
			"videoUrls",
			"https://www.youtube.com/watch?v=5vhVInexaUI,https://www.youtube.com/watch?v=CgXQC4dbGUE",
		);
	};
	...
	// 2. Define a submit postr.
	async function onSubmit(values: z.infer<typeof formSchema>) {
		const { post, title, description, videoUrls } = values;
		...
		try {
			const metadata = await getMetadataForPost(signer, post, true);
		...
		try {
			...
			const registerResult = await registerPost(
				signer,
				post,
				title,
				description || "",
			...
			//     args: [post, title, description || '', videoUrls || ''],
			// })
				...
				<Form {...form}>
					...
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									<FormLabel>Enter desired  post unique name</FormLabel>
									<FormControl>
										<Input placeholder={`Creator page post`} {...field} />
									</FormControl>
									<FormDescription>
										The post defines the unique url of the page. The post should be lower case
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									<FormLabel>Enter post title</FormLabel>
									<FormControl>
										<Input
									...
									<FormDescription>Post name</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									<FormLabel>Enter post description</FormLabel>
									<FormControl>
										<Textarea
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>
										Enter video urls to feature. These will be displayed on your profile
									</FormDescription>
									<FormMessage />
								</FormItem>
							)}
